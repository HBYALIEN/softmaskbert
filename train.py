...
class engines(bert):
    ...
    def train_model(self):
        ...
        model_save_path = '/home/WIN-UNI-DUE/smbohann/demo/softmaskgem/checkpoints'
        model_name = os.path.join(model_save_path, 'best_model_{}.pth'.format(time_stamp))
        model_name_d = os.path.join(model_save_path, 'best_model_d{}.pth'.format(time_stamp))
        print(model_name)
        print(model_name_d)
        ...
        for epoch in range(self.start_epoch, self.config.epoch):
            ...
            for i, batch_data in enumerate(self.train_data_loader):
                ...
                if total_loss < min_loss:
                    min_loss = total_loss
                    torch.save(self.detector_model.state_dict(), model_name_d)
    ...
    def test_model(self):
        ...
        for i, batch_data in enumerate(self.test_data_loader):
            ...
            a=[(output * batch_mask).reshape(-1)[j]for j in range(len(output.reshape(-1)))]
            for i in a:
                tokens =BertTokenizer.from_pretrained(self.config.vocab_file).convert_ids_to_tokens([i])
                string =BertTokenizer.from_pretrained(self.config.vocab_file).convert_tokens_to_string(tokens)
                words.append(string)
            print(words)
            corrector_correct = corrector_correct + sum(
                [(output * batch_mask).reshape(-1)[j].equal((batch_out_ids * batch_mask).reshape(-1)[j])
    ...
    def load_model(self):
        ...
        加载模型
        """
        self.detector_model.load_state_dict(torch.load('/home/WIN-UNI-DUE/smbohann/softmask/saved_models/best_model_d2022-06-16_09:00:37.pth'))  # 加载检测器模型参数
        #self.detector_optimizer.load_state_dict(self.checkpoint['detector_optimizer'])  # 加载检测器优化器参数
        self.model.load_state_dict(torch.load('/home/WIN-UNI-DUE/smbohann/softmask/saved_models/best_model_2022-06-16_09:00:37.pth'))  # 加载模型参数
